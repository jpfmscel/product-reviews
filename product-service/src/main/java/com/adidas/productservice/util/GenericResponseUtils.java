...
public class GenericResponseUtils {
	...
	public static GenericResponse buildGenericResponseError(Throwable t) {
		return GenericResponse.builder().status("error").code(400).messages(Arrays.asList(t.getCause().getMessage()))
				.build();
	}

	public static GenericResponse buildGenericResponseError(Object body) {
		return GenericResponse.builder().status("error").code(400).data(body).build();
	}

	public static GenericResponse buildGenericResponseError(String message, Object body) {
		return GenericResponse.builder().status("error").code(400).messages(Arrays.asList(message)).data(body).build();
	}
